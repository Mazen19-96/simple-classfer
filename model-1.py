# -*- coding: utf-8 -*-
"""
simple neural network to classfiction images of cats(binary-classfiction) :
the model of net is Single-layer-perceptron( Logistic Regression) 
"""
import numpy as np #is the fundamental package for scientific computing with Python.
import matplotlib.pyplot as plt #is a famous library to plot graphs in Python.
import h5py #is a common package to interact with a dataset that is stored on an H5 file.
import scipy #are used here to test your model with your own picture at the end.
from PIL import Image #are used here to test your model with your own picture at the end.
from scipy import ndimage 
from lr_utils import load_dataset # her we are import our dataset
...
# input the tarin and test dataset
...
# Reshape the training and test examples: her we are flatten our input data to fitting our net
...
must be specified for each pixel, 
...
def initialize_with_zeros(dim):
    ...
    return w, b
# test the parameter : wight and bias
dim = 2
w, b = initialize_with_zeros(dim)
print ("w = " + str(w))
print ("b = " + str(b))

#propagate pass:forward and backward propagation
...
def propagate(w, b, X, Y):
    ...
    cost = -(np.dot(Y,np.log(A.T)) +np.dot(np.log(1-A),(1-Y).T))/m  # we compute the cost function with:binary-cross-entropy                            
...
def optimize(w, b, X, Y, num_iterations, learning_rate, print_cost = False):
    ...
    for i in range(num_iterations):
        ...
        # Print the cost every  n training iterations
...
#test the optimizetion function
...
def predict(w, b, X):
    ...
    for i in range(A.shape[1]):
        ...
        if A[0][i]<=0.5:A[0][i]=0  #threshlod = 0.5
        else: A[0][i]=1
    Y_prediction=A
